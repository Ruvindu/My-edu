...
<?php 
  ...
  /*  CHECK USER IS VALID USER */
    ...
    $user_qurey = "SELECT * FROM `user` WHERE `id` = '{$member_id}' LIMIT 1";
    ...
    $get_privacy = "SELECT * FROM `privacy` WHERE `user_id` = {$member_id}";
	...
	<div class="container-fluid">
     ...
     <div class="row">
        ...
        <div class="col-md-12">
          ...
          <div id="search">
                  <form action="member_profile.php?" method="post" autocomplete="on">
                    <input type="hidden" name="member_id" <?php echo "value={$member_id}"; ?> >
    ...
    <div class="row">
              ...
              <table class="p_about bgtheme" >
                  ...
                  <tr>
                    <td colspan="2" class="my_img" align="center">
                       ...
                       <?php echo "<img class=\"dp_about\" src=\"{$user_details['picture']}\" alt=\"profile image\"  width=\"140px\" height=\"140px\">" ?>
                    </td>
                  </tr>
                  ...
                  <tr>
                    <th class="sub_topic">Birthday</th> <td><?php if(strcmp($privacy['dob'], "full")==0){ echo $user_details['dob']; } else{

                        $date = strtotime($user_details['dob']);

                        $newformat = date('M d',$date);
                  ...
                  <tr>
                    <th class="sub_topic">Email</th> <td><?php if(strcmp($privacy['email'], "public")==0){ echo $user_details['email']; } ?></td>
                  </tr>

                  <tr>
                    <th class="sub_topic">Phone</th> <td><?php if(strcmp($privacy['phone'], "public")==0){  echo $user_details['phone']; } ?></td>
                  </tr>

                  <tr>
                    <th class="sub_topic">City</th> <td><?php if(strcmp($privacy['city'], "public")==0){ echo $user_details['city']; } ?></td>
                  </tr>

                  <tr>
                    <th class="sub_topic">Country</th> <td><?php if(strcmp($privacy['country'], "public")==0){ echo $user_details['country']; } ?></td>
                  </tr>

                  <tr>
                    <th class="sub_topic">Joined</th> <td><?php if(strcmp($privacy['join_date'], "public")==0){ echo $user_details['joined']; } ?></td>
                  </tr>
      ...
      <?php        
            ...
            if (isset($_POST['search']) && $_POST['search_text']!="" ) {
                ...
                $getmy_docs = "SELECT `doc_id`, `owner_id`, `permission`, `date_time`, `doc_heading`, `valid_for`, `doc_type`, `doc_media`, `doc_dec`, `path`, `is_deleted` FROM `documents` WHERE  `owner_id` = {$user_details['id']}  AND `is_deleted` = 0 AND `permission` = 'public' AND `doc_heading` LIKE '%{$search_text}%' OR `doc_dec` LIKE '%{$search_text}%' ORDER BY doc_id DESC ";
              }else{

                $getmy_docs = "SELECT `doc_id`, `owner_id`, `permission`, `date_time`, `doc_heading`, `valid_for`, `doc_type`, `doc_media`, `doc_dec`, `path`, `is_deleted` FROM `documents` WHERE `owner_id` = {$user_details['id']}  AND `is_deleted` = 0  AND `permission` = 'public' ORDER BY doc_id DESC ";
